...
export function LoginForm() {
  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    if (error) {
      setServerError("Hmm, those details don't match. Please try again.");
      setIsLoading(false);
      return;
    }
  ...
  const handleGoogleSignIn = async () => {
    await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: { redirectTo: `${window.location.origin}/cycle` },
    });
  };
  ...
  return (
    <div className="min-h-screen bg-cream flex items-center justify-center px-4 py-16 relative overflow-hidden">
      {/* Background blobs */}
      <div className="absolute -top-20 -left-20 w-80 h-80 rounded-full bg-blush/40 blur-3xl -z-10" />
      <div className="absolute -bottom-20 -right-20 w-96 h-96 rounded-full bg-lavender/30 blur-3xl -z-10" />
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-xl border border-sage/20 p-8 md:p-10">
          <div className="text-center mb-8">
            ...
            <p className="text-plum/55 text-sm">
              Good to see you again. Your body has been busy — let&rsquo;s catch up.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <span className="text-plum/35 text-xs font-medium">or log in with email</span>
            <div className="flex-1 h-px bg-sage/30" />
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="block text-sm font-medium text-plum/80">Password</label>
                <button
                  type="button"
